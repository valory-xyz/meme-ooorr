#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ------------------------------------------------------------------------------
#
#   Copyright 2021-2024 David Vilela Freire
#
#   Licensed under the Apache License, Version 2.0 (the "License");
#   you may not use this file except in compliance with the License.
#   You may obtain a copy of the License at
#
#       http://www.apache.org/licenses/LICENSE-2.0
#
#   Unless required by applicable law or agreed to in writing, software
#   distributed under the License is distributed on an "AS IS" BASIS,
#   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#   See the License for the specific language governing permissions and
#   limitations under the License.
#
# ------------------------------------------------------------------------------

"""Tweepy wrapper."""

import logging
import re
from typing import Dict, List, Optional

import tweepy  # type: ignore[import]


DEFAULT_LOGGER = logging.getLogger(__name__)


def is_twitter_id(twitter_id: str) -> bool:
    """
    Check if a string is a valid Twitter ID.

    Args:
        twitter_id (str): The string to check.

    Returns:
        bool: True if the string is a valid Twitter ID, False otherwise.
    """
    return bool(re.match(r"^\d{1,20}$", twitter_id))


class Twitter:
    """A class to interact with Twitter API using Tweepy."""

    def __init__(
        self,
        consumer_key: str,
        consumer_secret: str,
        access_token: str,
        access_token_secret: str,
        bearer_token: str,
        logger: Optional[logging.Logger] = None,
    ):
        """Constructor"""
        self.oauth2_bearer_auth = tweepy.OAuth2BearerHandler(bearer_token)
        self.oauth2_app_auth = tweepy.OAuth2AppHandler(consumer_key, consumer_secret)
        self.oauth1_user_auth = tweepy.OAuth1UserHandler(
            consumer_key=consumer_key,
            consumer_secret=consumer_secret,
            access_token=access_token,
            access_token_secret=access_token_secret,
        )

        self.api = tweepy.API(auth=self.oauth1_user_auth)

        self.client = tweepy.Client(
            bearer_token=bearer_token,
            consumer_key=consumer_key,
            consumer_secret=consumer_secret,
            access_token=access_token,
            access_token_secret=access_token_secret,
        )
        self.logger = logger if logger else DEFAULT_LOGGER

    def post_tweet(
        self,
        text: str,
        image_paths: Optional[List[str]] = None,
        in_reply_to_tweet_id: Optional[int] = None,
    ) -> Optional[str]:
        """
        Posts a new tweet with optional media.
        """
        method_name = "post_tweet"
        try:
            tweet = self.client.create_tweet(
                text=text,
                media_ids=(
                    [
                        self.api.media_upload(filename=image_path).media_id
                        for image_path in image_paths
                    ]
                    if image_paths
                    else None
                ),
                in_reply_to_tweet_id=in_reply_to_tweet_id,
            )
            return tweet.data["id"]
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return None

    def delete_tweet(self, tweet_id: str) -> bool:
        """
        Deletes a specific tweet.
        """
        method_name = "delete_tweet"
        try:
            self.client.delete_tweet(tweet_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def like_tweet(self, tweet_id: str) -> bool:
        """
        Likes a specific tweet.
        """
        method_name = "like_tweet"
        try:
            self.client.like(tweet_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def unlike_tweet(self, tweet_id: str) -> bool:
        """
        Unlikes a specific tweet.
        """
        method_name = "unlike_tweet"
        try:
            self.client.unlike(tweet_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def retweet(self, tweet_id: str) -> bool:
        """
        Retweets a specific tweet.
        """
        method_name = "retweet"
        try:
            self.client.retweet(tweet_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def unretweet(self, tweet_id: str) -> bool:
        """
        Unretweets a specific tweet.
        """
        method_name = "unretweet"
        try:
            self.client.unretweet(tweet_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def follow_by_id(self, user_id: str) -> bool:
        """
        Follow a specific user.
        """
        method_name = "follow_by_id"
        try:
            self.client.follow(user_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def follow_by_username(self, username: str) -> bool:
        """
        Follow a specific user by username.
        """
        method_name = "follow_by_username"
        user_id = self.get_user_id(username=username)

        if not user_id:
            self.logger.error(
                f"Could not follow user by username '{username}' in '{method_name}' because their ID could not be retrieved (get_user_id failed)."
            )
            return False

        return self.follow_by_id(user_id)

    def unfollow_by_id(self, user_id: str) -> bool:
        """
        Unfollow a specific user.
        """
        method_name = "unfollow_by_id"
        try:
            self.client.unfollow(user_id)
            return True
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return False

    def get_user_id(self, username: str) -> Optional[str]:
        """
        Get a user id.
        """
        method_name = "get_user_id"
        try:
            user = self.client.get_user(username=username)
            return user.data.id
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return None

    def get_me(self) -> Optional[Dict]:
        """
        Get my user.
        """
        method_name = "get_me"
        try:
            result = self.client.get_me()
            return {"user_id": result.data.id, "username": result.data.username}
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}': {type(e).__name__} - {e}"
            )
            return None

    def get_follower_ids(self, user: str) -> Optional[List[str]]:
        """
        Get a list of follower ids.
        """
        method_name = "get_follower_ids"

        user_id = user if is_twitter_id(user) else self.get_user_id(user)

        if not user_id:
            self.logger.error(
                f"Could not get follower IDs for '{user}' in '{method_name}' because their ID could not be retrieved (get_user_id failed)."
            )
            return None
        try:
            result = self.api.get_follower_ids(user_id=user_id)
            return result
        except tweepy.errors.TweepyException as e:
            self.logger.error(
                f"TweepyException in method '{method_name}' during self.api.get_follower_ids: {type(e).__name__} - {e}"
            )
            return None
