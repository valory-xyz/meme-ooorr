...
from packages.valory.skills.memeooorr_abci.models import Params, SharedState
from packages.valory.skills.memeooorr_abci.rounds import SynchronizedData
...
class MemeooorrBaseBehaviour(
    ...
    def get_meme_coins_from_subgraph(self) -> Generator[None, None, Optional[List]]:
        ...
        items = ((response_json.get("data") or {}).get("memeTokens") or {}).get(
            "items"
        ) or []
        if not items:
    ...
    def replace_tweet_with_alternative_model(  # pylint: disable=too-many-return-statements
        self, prompt: str
    ) -> Generator[None, None, Optional[str]]:
        ...
        try:
            ...
            self.context.logger.error(
                f"Fireworks returned non-JSON body: {response.body!r}"
            )
            return None
